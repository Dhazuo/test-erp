    ...
    <div class="p-2 grid grid-cols-1 gap-y-8 text-sm">
        ...
        <div class="w-full max-h-full bg-white rounded-md p-2 font-bold grid grid-cols-1 gap-y-4">
            <div class="grid grid-cols-1 gap-y-2">
                ...
                <input v-model="temporal_products" autoComplete="on" list="product" class="p-2 rounded-md border border-gray-200 shadow-sm focus:outline-none focus:border-main transition duration-200"/>
            </div>
            <div class="grid grid-cols-2 gap-x-4">
                <div class="grid grid-cols-1 gap-y-2">
                    ...
                    <input v-model="form.customer" autoComplete="on" list="customer" class="p-2 rounded-md border border-gray-200 shadow-sm focus:outline-none focus:border-main transition duration-200"/>
                </div>
                <div class="grid grid-cols-1 gap-y-2">
                    <label for="seller">Selecciona tu vendedor:</label>
                    <datalist id="seller">
                        <option v-for="seller in sellers" :value="seller.name">
                        </option>
                    </datalist>
                    <input v-model="form.seller" autoComplete="on" list="seller" class="p-2 rounded-md border border-gray-200 shadow-sm focus:outline-none focus:border-main transition duration-200"/>
                </div>
            </div>
            <div class="grid grid-cols-1 gap-y-2">
                ...
                <div class="w-full">
                    <table class="table-auto w-full">
                        ...
                        <tr>
                            ...
                            <td class="p-2 text-center">Unitario</td>
                        ...
                        <tr v-for="(product, index) in form.products" class="border-b border-gray-200">
                            ...
                            <td class="p-2 flex justify-center">
                                <input v-model="product.stock" type="number" class="w-24 text-center text-sm focus:ring-0 rounded-md outline-none border border-gray-200 shadow-sm focus:outline-none focus:border-main transition duration-200">
                            </td>
                            ...
                            <td>
                                <button @click="deleteFromProducts(index)" class="rounded-full shadow-sm bg-red-500 h-6 w-6 text-white text-center flex justify-center items-center font-bold hover:bg-red-600 transition duration-200 text-xs">
                                    X
                                </button>
                            </td>
                        </tr>
                        <tr v-if="form.products.length > 0">
                            <td></td>
                            <td></td>
                            <td class="text-center text-main">TOTAL: </td>
            ...
            <div class="w-full flex justify-center">
                <button :disabled="is_processing" @click="calculate" class="px-4 py-2 bg-main rounded-md shadow-md text-white font-bold">
...
export default {
    ...
    props: ['products', 'sellers', 'customers', 'sale'],
    data() {
        return {
            form: {
                seller: this.sale.seller_name,
    ...
    methods: {
        ...
        save() {
            axios.put(this.route('management.sales.update', this.sale.id), this.form)
                .then(response => {
        ...
        verification(){
            ...
            let find_seller = this.sellers.findIndex(element => {
                return element.name == this.form.seller
            })
            if (find_customer < 0 || find_seller < 0) {
                if (find_customer < 0) {
    ...
    mounted() {
        this.sale.products.forEach(product => {
            this.form.products.push({
                name: product.name,
                stock: product.pivot.total_stock,
                price: product.price
            })
        })
    }
