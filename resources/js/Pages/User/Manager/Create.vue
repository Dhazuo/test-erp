            ...
            <div class="w-2/4 rounded-md shadow-md border border-gray-200 grid grid-cols-1">
                ...
                <div class="grid grid-cols-1 gap-y-6 p-2">
                    ...
                    <div class="w-full flex justify-center">
                        <div class="text-center grid grid-cols-1 gap-y-2">
                            <div>Rol</div>
                            <div class="px-4 py-2 border border-gray-200 shadow-sm rounded-md cursor-not-allowed">
                                seller
                            </div>
                        </div>
                    </div>
...
export default {
    ...
    methods: {
        save() {
            axios.post(this.route('management.user.store'), this.form)
                .then(response => {
                    window.location = this.route('management.user.show', response.data.id)
                })
        },
