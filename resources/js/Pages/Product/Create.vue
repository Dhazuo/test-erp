    ...
    <div class="p-2 grid grid-cols-1 gap-y-8">
        ...
        <div class="w-full flex justify-center ">
            <div class="w-2/4 rounded-md shadow-md border border-gray-200 grid grid-cols-1">
                <div class="p-2 bg-gray-100 text-gray-400 text-2xl border-b border-gray-500 text-center">Creando
                    producto
                </div>
                    ...
                    <div class="grid grid-cols-2 gap-x-6">
                        ...
                        <div class="text-center grid grid-cols-1 gap-y-2">
                            ...
                            <div class="flex justify-center">
                                <div
                                    class="h-auto flex items-center justify-center content-center bg-gray-100 px-2 rounded-l-md">
                                    $
                                </div>
                                <input v-model="form.price" id="price" type="number" step="any"
                                       class="text-center rounded-r-md shadow-sm border border-gray-200">
                            </div>
                        </div>
                        <div class="text-center grid grid-cols-1 gap-y-2">
                            <label for="stock">Stock</label>
                            <input v-model="form.original_stock" id="stock" type="number"
                                   class="text-center rounded-md shadow-sm border border-gray-200">
                        </div>
                    </div>
                    <div class="w-full flex justify-center">
                        <button @click="save" class="px-4 py-2 bg-blue-500 text-white shadow-md rounded-md">Guardar
                        </button>
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods: {
        save() {
            ...
            axios.post(this.route('management.product.store'), this.form)
                .then(response => {
                    window.location = this.route('management.product.show', response.data.id)
                })
        },
        convert() {
